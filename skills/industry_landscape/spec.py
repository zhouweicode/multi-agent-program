...
INDUSTRY_LANDSCAPE_SPEC = SkillSpec(
    skill_id="industry_landscape",
    version="1.0.0",
    name="产业全景报告",
    description="基于产业链结构、关联企业、重点事件和可选公开来源生成可追溯产业全景报告。",
    trigger_phrases=("产业全景报告", "产业链全景报告", "行业全景报告", "产业研究报告"),
    required_capabilities=("industry_landscape_core",),
    optional_capabilities=("external_industry_evidence",),
    output_schema="IndustryLandscapeReport",
...
def _industry_query(question: str) -> str:
    match = re.search(r"([\u4e00-\u9fffA-Za-z0-9_-]{2,30}?)(?:产业链|产业|行业)(?:全景|研究)?报告", question)
    if not match:
        return ""
    value = match.group(1)
    for prefix in ("请生成", "生成", "请输出", "输出", "请做一份", "做一份", "一份"):
        value = value.removeprefix(prefix)
    for modifier in ("面向投资人的", "面向投资人", "面向政府的", "面向政府", "面向企业的", "面向企业",
                     "简版", "简要", "摘要版", "精简", "完整", "综合"):
...
def normalize_industry_landscape_input(question: str, raw: dict[str, Any] | None = None) -> dict[str, Any]:
